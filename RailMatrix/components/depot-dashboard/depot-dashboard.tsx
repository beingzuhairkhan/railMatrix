...
export function DepotDashboard({ userData, accessToken, onLogout }: DepotDashboardProps) {
  const { t } = useLanguage()
    ...
    <div className="min-h-screen bg-background">
      ...
      <header className="border-b bg-card">
        <div className="flex h-16 items-center justify-between px-6">
          <div>
            <h1 className="text-xl font-bold text-foreground">{t("depot.title")}</h1>
            <p className="text-sm text-muted-foreground">Welcome, {userData.employee_id}</p>
          </div>
                ...
                {inventory.map((item) => (
                  <Card key={item._id}>
                    ...
                    <CardContent>
                      ...
                      <Progress value={(item.available / (item.reorder_level || 1)) * 100} className="mt-2" />
                    </CardContent>
                  </Card>
                ))}
              </div>
              ...
              <Card key={order.id} className="mb-2">
                <CardContent className="flex justify-between items-center">
                  ...
                  {order.status !== "Delivered" && (
                    <Button
                      onClick={() => handleDispatchUpdate(order.id, order.status === "Ready" ? "In Transit" : "Delivered")}
              ...
              <Card key={claim.id} className="mb-2">
                <CardContent className="flex justify-between items-center">
                  ...
                  {claim.status === "Under Review" && (
                    <Button onClick={() => handleWarrantyClaim(claim.id)} disabled={isProcessingWarranty === claim.id}>
